...
PERIODO_DEFAULT   = "1y"
MIN_VELAS         = 50
CACHE_TIMEOUT_SEG = 1800  # 30 min — reduce peticiones a Yahoo Finance
...
def _desde_yfinance(ticker: str, fecha_inicio: datetime, fecha_fin: datetime) -> Optional[pd.DataFrame]:
    ...
    for intento in range(2):
        try:
            ...
            if intento == 0:
                logger.warning(f"⚠️  yfinance intento 1 falló para {ticker}: {e} — reintentando...")
                time.sleep(2)
            else:
...
def get_df_ibex(cache=None, periodo: str = "1y") -> Optional[pd.DataFrame]:
    ...
    if df is not None and not df.empty:
        return df
    # Fallback: ticker alternativo en Yahoo Finance
    logger.warning("⚠️ ^IBEX falló — intentando fallback ^IBEX.MC")
    df = get_df("^IBEX.MC", periodo=periodo, cache=cache, min_velas=210)
    if df is not None and not df.empty:
        return df
    # Segundo fallback: IBE (Iberdrola como proxy del IBEX no ideal pero operativo)
    logger.warning("⚠️ ^IBEX.MC falló — contexto de mercado sin datos IBEX")
    return None
