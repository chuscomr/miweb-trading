...
def calcular_rr(entrada: float, stop: float, objetivo: float, rr_minimo: float = 1.5) -> Optional[float]:
    ...
    MEJORA v82.5: Validación de RR mínimo
    
    Args:
        entrada: Precio de entrada
        stop: Precio de stop
        objetivo: Precio objetivo
        rr_minimo: RR mínimo aceptable (default: 1.5)
        
    Returns:
        float RR (ej: 2.5) o None si:
        - Los datos son inválidos
        - RR < rr_minimo (no tiene ventaja matemática)
    
    Fundamento matemático:
    ├─ RR < 1.5 requiere winrate > 67% (muy restrictivo)
    ├─ RR = 1.5 requiere winrate > 40% (mínimo sano)
    ├─ RR >= 2.0 requiere winrate > 33% (cómodo)
    └─ RR >= 3.0 requiere winrate > 25% (excelente)
    """
