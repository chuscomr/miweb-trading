...
def multiplicador_score_mercado(cache=None) -> float:
    """
    Multiplicador de setup_score según contexto de mercado.
    
    FILOSOFÍA: No rechaza setups en mercados bajistas,
    sino que reduce su score para priorizar los alcistas.
    
    Esto permite capturar "giros de mercado" (los mejores trades)
    mientras se protege de ruido.
    
    Returns:
        ALCISTA    → 1.0   (score sin cambios)
        TRANSICION → 0.85  (reduce 15%)
        BAJISTA    → 0.7   (reduce 30%, pero no rechaza)
    
    Uso:
        score_ajustado = setup_score * multiplicador_score_mercado(cache)
    """
    ctx = evaluar_contexto_ibex(cache)
    return {
        ESTADO_ALCISTA:    1.0,
