...
def _calcular_benchmark_ibex(df_semanal):
    """
    Calcula el retorno Buy & Hold del IBEX35 en el mismo período que el backtest.
    ...
    try:
        ...
        if isinstance(df_ibex.columns, pd.MultiIndex):
            df_ibex.columns = df_ibex.columns.get_level_values(0)

        precio_ini = float(df_ibex["Close"].iloc[0])
        precio_fin = float(df_ibex["Close"].iloc[-1])
        retorno_pct = round((precio_fin - precio_ini) / precio_ini * 100, 2)
        años = (fecha_fin - fecha_ini).days / 365.25
        retorno_anual = round((((precio_fin / precio_ini) ** (1/años)) - 1) * 100, 2) if años > 0 else 0

        return {
            "retorno_pct":    retorno_pct,
            "retorno_anual":  retorno_anual,
            "precio_inicio":  round(precio_ini, 2),
...
def ejecutar_backtest_medio_plazo(df_semanal, ticker=None, verbose=False):
    ...
    metricas = _calcular_metricas(trades, equity_curve)

    # ── Benchmark IBEX — Buy & Hold en el mismo período ──────────────────────
    benchmark = _calcular_benchmark_ibex(df_semanal)

    return {"trades": trades, "equity": equity_curve,
            "metricas": metricas, "benchmark": benchmark}
