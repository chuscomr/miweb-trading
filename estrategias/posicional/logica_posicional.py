...
def detectar_consolidacion(precios, lookback_max=26):
    ...
    CORRECCIÓN LOOKAHEAD (v82+): Excluye la vela actual (información futura)
    """
    ...
    # CORRECCIÓN: excluye el precio actual [-1] para evitar lookahead
    periodo_consolidacion = precios[-(lookback_max+1):-1]
    if len(periodo_consolidacion) == 0:
        return {
            "en_consolidacion": False,
            "motivo": "Histórico insuficiente"
        }
...
def calcular_stop_inicial(entrada, precios, df=None):
    ...
    MEJORA v82.4: Stop híbrido mejorado con validación

    Usa el MAYOR de:
    - Mínimo últimas 26 semanas × 0.98 (estructura técnica)
    - Entrada - (ATR × 2.5) (volatilidad)
    
    Validaciones:
    - No permite stops < 6% ni > 15% (riesgo controlado)
    - Valida que estructura sea sólida (no mínimos extremos)
    - Fallback conservador si ATR no disponible
    ...
    if len(precios) >= STOP_ESTRUCTURA_LOOKBACK:
        if df is not None and 'Low' in df.columns:
            # Excluye el registro actual para evitar lookahead
            minimo_estructura = df['Low'].iloc[-(STOP_ESTRUCTURA_LOOKBACK + 1):-1].min()
        else:
            # Excluye el precio actual
            minimo_estructura = min(precios[-(STOP_ESTRUCTURA_LOOKBACK + 1):-1])
    else:
        # Fallback: usa todos excepto el actual, con margen
        minimo_estructura = min(precios[:-1]) * 0.85

    # Aplicar margen a estructura (0.98 = protege 2% contra whipsaws)
    stop_estructura = minimo_estructura * 0.98

    # Calcular stop por ATR (volatilidad)
    stop_atr = entrada
    if df is not None:
        atr = calcular_atr(df, ATR_PERIODO)
        if atr and atr > 0:
            # ATR × 2.5 da buffer contra ruido pero respeta volatilidad
            stop_atr = entrada - (STOP_ATR_MULTIPLICADOR * atr)

    # STOP HÍBRIDO: Toma el MAYOR (más conservador)
    # Esto evita stops muy ajustados (ruido) o muy amplios (mal riesgo/recompensa)
    stop = max(stop_estructura, stop_atr)

    riesgo_pct = ((entrada - stop) / entrada) * 100

    # Validación: Mantener riesgo en rango sano (6-15%)
