...
def ejecutar_backtest_posicional(df_semanal, ticker, verbose=False, df_ibex=None):
    ...
    try:
        ...
        if df_ibex is not None and not df_ibex.empty:
            ...
            p_ini = float(df_ibex["Close"].iloc[0])
            p_fin = float(df_ibex["Close"].iloc[-1])
            ret_pct = round((p_fin - p_ini) / p_ini * 100, 2)
            años = (fecha_fin - fecha_ini).days / 365.25
            ret_anual = round((((p_fin/p_ini)**(1/años))-1)*100, 2) if años > 0 else 0
            metricas["benchmark"] = {
                "retorno_pct":   ret_pct,
                "retorno_anual": ret_anual,
                "fecha_inicio":  str(fecha_ini.date()) if hasattr(fecha_ini, 'date') else str(fecha_ini),
                "fecha_fin":     str(fecha_fin.date())  if hasattr(fecha_fin,  'date') else str(fecha_fin),
                "años":          round(años, 1),
            }
        else:
            metricas["benchmark"] = None
    except Exception:
        metricas["benchmark"] = None
