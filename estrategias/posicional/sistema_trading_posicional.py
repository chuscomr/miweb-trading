...
def calcular_score_posicional(detalles, fr_diff=0, fr_cat="SIN_DATOS"):
    ...
    # 8. S/R — Calidad breakout + espacio libre (suma Y resta)
    # Se aplica sobre el df si está disponible en detalles
    df_pos = detalles.get("df", None)
    if df_pos is not None and len(df_pos) >= 40:
        try:
            from analisis.tecnico.soportes_resistencias import evaluar_sr
            sr = evaluar_sr(df_pos, periodo=15, timeframe="semanal")
            pts_sr   = sr["score_sr"]   # puede ser negativo
            desc_sr  = sr["resumen"]
            score_desglose["sr"] = (pts_sr, 4, desc_sr)
            total += pts_sr
