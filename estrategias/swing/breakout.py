...
Criterios (ajustados por Salva):
  1. Precio cerca del máximo 20d  (≥ -3%)
...
class BreakoutSwing(EstrategiaBase):
    ...
    def _evaluar_df(self, df: pd.DataFrame, ticker: str) -> dict:
        ...
        # ══════════════════════════════════════════════════
        # 1️⃣  PRECIO CERCA DEL MÁXIMO 20d  (≥ -3%)
        # ══════════════════════════════════════════════════
        maximo_20         = f(df["Close"].tail(20).max())
        dist_maximo_pct   = ((precio_actual - maximo_20) / maximo_20) * 100
        precio_max_ok     = dist_maximo_pct >= -3.0

        motivos.append({
            "ok":    precio_max_ok,
            "texto": f"Precio cerca máximo 20d ({dist_maximo_pct:.2f}%)"
        })

        # ══════════════════════════════════════════════════
        # 2️⃣  RESISTENCIA PRINCIPAL  (-3% / +6%)
        # ══════════════════════════════════════════════════
        resistencias = _identificar_resistencias(df.tail(60))
        ...
        resistencia_ok = -3.0 <= dist_resistencia_pct <= 6.0
        ...
        # ══════════════════════════════════════════════════
        # 5️⃣  RSI MOMENTUM  (50 – 82)
        # ══════════════════════════════════════════════════
        rsi_ok = 50 <= rsi_val <= 82
        ...
        # PLAN DE TRADING
        # ══════════════════════════════════════════════════
        min_consolidacion = f(df["Low"].tail(consolidacion_dias).min())
        ...
        return respuesta_valida(
            ...
            # Campos extra específicos de BREAKOUT
