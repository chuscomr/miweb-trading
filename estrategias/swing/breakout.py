...
class BreakoutSwing(EstrategiaBase):
    ...
    def _evaluar_df(self, df: pd.DataFrame, ticker: str) -> dict:
        ...
        maximo_52         = f(df["Close"].iloc[:-1].tail(52).max())
        ...
        score_parcial = sum(peso for _, ok, peso in PESOS_OPC if ok)
        ...
        min_consolidacion = f(df["Low"].iloc[:-1].tail(lookback_consol).min())
