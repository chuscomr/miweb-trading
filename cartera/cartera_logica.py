...
class CarteraLogica:
    ...
    def calcular_resumen(self, posiciones: list, config: dict, cerradas_mes: list) -> dict:
        ...
        # ── Drawdown operativo — alerta si supera umbral ─────────
        # Un sistema bueno debe ser psicológicamente soportable
        # Regla: si la cartera cae >X% desde máximo reciente → PARAR
        DRAWDOWN_ALERTA_PCT  = 10.0  # Amarillo — revisar
        DRAWDOWN_PARAR_PCT   = 15.0  # Rojo — dejar de operar

        # Drawdown actual = pérdida flotante sobre capital
        drawdown_actual_pct  = round(-pnl_flotante / capital * 100, 2) if capital and pnl_flotante < 0 else 0.0
        drawdown_estado      = "OK"
        if drawdown_actual_pct >= DRAWDOWN_PARAR_PCT:
            drawdown_estado = "PARAR"
            alertas.append({
                "tipo": "ko",
                "texto": f"⛔ DRAWDOWN {drawdown_actual_pct:.1f}% — Sistema en pausa. No abrir nuevas posiciones."
            })
        elif drawdown_actual_pct >= DRAWDOWN_ALERTA_PCT:
            drawdown_estado = "ALERTA"
            alertas.append({
                "tipo": "warn",
                "texto": f"⚠️ Drawdown {drawdown_actual_pct:.1f}% — Revisar sistema. Reducir tamaño de posición."
            })
        ...
        return {
            ...
            "drawdown_estado":     drawdown_estado,
            "drawdown_alerta_pct": DRAWDOWN_ALERTA_PCT,
            "drawdown_parar_pct":  DRAWDOWN_PARAR_PCT,
        }
