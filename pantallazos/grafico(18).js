...
class GraficoIndicadores {
    ...
    dibujar(data, indicadores) {
        ...
        const GAP = 0.01;
        const pVolumen = tieneVolumen ? 0.12 : 0;
        const pMACD = tieneMACD ? 0.10 : 0;
        const pOBV = tieneOBV ? 0.10 : 0;
        const pRSI = tieneRSI ? 0.10 : 0;
        const pMFI = tieneMFI ? 0.10 : 0;
        const pADX = tieneADX ? 0.10 : 0;
        const pATR = tieneATR ? 0.10 : 0;
        ...
        // ── ALTURA DINÁMICA ──────────────────────────────────────────
        // Base proporcional al viewport: en pantallas grandes el gráfico
        // debe ser más alto para mantener proporciones visuales correctas
        const VW = window.innerWidth;
        const BASE_HEIGHT = Math.round(Math.max(700, VW * 0.38));  // ~38% del viewport
        const PX_POR_PANEL = Math.round(VW * 0.075);               // ~7.5% del viewport por panel
        const nPaneles = [tieneVolumen, tieneMACD, tieneOBV, tieneRSI, tieneMFI, tieneADX, tieneATR]
            .filter(Boolean).length;
        const alturaTotal = BASE_HEIGHT + nPaneles * PX_POR_PANEL;
        const elGrafico = document.getElementById('grafico');
        elGrafico.style.height = alturaTotal + 'px';

        // Precio ocupa mínimo 55% — los paneles se reparten el resto
        const MIN_PRECIO = 0.55;
        let escala = 1.0;
        ...
        const layout = {
            ...
            height: alturaTotal,   // ← CLAVE: Plotly necesita su propio height, no solo el del div
