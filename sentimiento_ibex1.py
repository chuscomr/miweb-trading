...
sentimiento_ibex.py
─────────────────────────────────────────────────────────────────
Análisis de sentimiento IBEX 35.
Fuentes: Bing News RSS + Google News RSS (fallback)
...
def _fetch_noticias(emp: dict) -> list:
    ...
    noticias = _fetch_bing(f"{nombre} bolsa acciones")
    if not noticias:
        noticias = _fetch_google(f"{nombre} bolsa acciones")
    if not noticias:
        noticias = _fetch_bing(f"{ticker} IBEX bolsa")

    # Solo noticias con título y url
