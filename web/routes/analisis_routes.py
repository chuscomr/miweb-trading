...
@analisis_bp.route("/sentimiento", methods=["GET"])
def sentimiento_panel():
    """Página de análisis de sentimiento IBEX 35"""
...
# ─────────────────────────────────────────────────────────────
# SENTIMIENTO TÉCNICO POR EMPRESA
# GET /analisis/api/sentimiento/<ticker>
# ─────────────────────────────────────────────────────────────

@analisis_bp.route("/api/sentimiento/<ticker>")
def api_sentimiento_empresa(ticker):
    """Calcula el sentimiento técnico de mercado para un ticker."""
    from core.universos import normalizar_ticker
    from sentimiento_empresa import calcular_sentimiento_empresa
    ticker = normalizar_ticker(ticker)
    cache  = _get_cache()
    cache_key = f"sentimiento_tecnico_{ticker}"
    resultado = cache.get(cache_key) if cache else None
    if resultado is None:
        resultado = calcular_sentimiento_empresa(ticker, cache)
        if cache and not resultado.get("error"):
            cache.set(cache_key, resultado, timeout=600)
    return jsonify(resultado)


# ─────────────────────────────────────────────────────────────
# SENTIMIENTO IBEX COMPLETO (todos los tickers)
# GET /analisis/api/sentimiento_ibex
# ─────────────────────────────────────────────────────────────

@analisis_bp.route("/api/sentimiento_ibex")
def api_sentimiento_ibex():
    """Score de sentimiento técnico para todo el IBEX 35."""
    from core.universos import IBEX35, get_nombre
    from sentimiento_empresa import calcular_sentimiento_empresa
    cache = _get_cache()
    cache_key = "sentimiento_ibex_tecnico"
    resultado = cache.get(cache_key) if cache else None
    if resultado is None:
        empresas = []
        scores   = []
        for ticker in IBEX35:
            r = calcular_sentimiento_empresa(ticker, cache)
            if not r.get("error"):
                empresas.append({
                    "ticker":    ticker,
        ...
        score_ibex = round(sum(scores) / len(scores)) if scores else 0
        alcistas   = sum(1 for s in scores if s >= 15)
        bajistas   = sum(1 for s in scores if s <= -15)
        neutras    = len(scores) - alcistas - bajistas
        resultado  = {
            "ok": True,
            "empresas": sorted(empresas, key=lambda x: x["score"], reverse=True),
            "score_ibex": score_ibex,
            "alcistas": alcistas,
            "bajistas": bajistas,
            "neutras":  neutras,
        }
...
# ─────────────────────────────────────────────────────────────
# SENTIMIENTO REAL DE MERCADO (short interest + analistas + precio + volumen + prensa)
# GET /analisis/api/sentimiento_real/<ticker>
# ─────────────────────────────────────────────────────────────

@analisis_bp.route("/api/sentimiento_real/<ticker>")
def api_sentimiento_real(ticker):
    """Sentimiento real de mercado: short interest, analistas, precio vs objetivo, volumen, prensa."""
    from core.universos import normalizar_ticker
    from sentimiento_mercado import calcular_sentimiento_mercado
    ticker    = normalizar_ticker(ticker)
    cache     = _get_cache()
    cache_key = f"sent_real_{ticker}"
    resultado = cache.get(cache_key) if cache else None
    if resultado is None:
        resultado = calcular_sentimiento_mercado(ticker, cache)
        if cache and not resultado.get("error"):
...
@analisis_bp.route("/api/insiders/<ticker>")
def api_insiders_get(ticker):
    """
    Lee insiders desde DB local (con caducidad).
    ...
    try:
        ...
        if not ticker.endswith(".MC"):
            ticker += ".MC"
        resultado = get_insiders(ticker)
        return jsonify({"ok": True, "ticker": ticker, **resultado})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500


@analisis_bp.route("/api/insiders/scrape/<ticker>", methods=["POST"])
def api_insiders_scrape(ticker):
    """
    Lanza scraping CNMV para un ticker específico.
    SEPARADO de la lectura — el usuario lo invoca manualmente.
    Puede tardar varios segundos (request HTTP a CNMV).
    """
    from analisis.fundamental.insiders import get_insiders, scrape_insiders
    try:
        ticker  = ticker.upper()
        ...
        res_scrape = scrape_insiders(ticker, forzar=forzar)
        # Devolver evaluación actualizada
        resultado  = get_insiders(ticker)
        return jsonify({
            "ok":      True,
            "ticker":  ticker,
            "scrape":  res_scrape["status"],
            "mensaje": res_scrape["mensaje"],
            **resultado,
        })
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
@analisis_bp.route("/api/insiders/scrape_batch", methods=["POST"])
def api_insiders_scrape_batch():
    """
    Scraping en lote de todos los tickers con ISIN conocido.
    ...
    try:
        ...
        for t in tickers:
            try:
                res = scrape_insiders(t, forzar=False)
                resultados[res.get("status", "error")] = resultados.get(res.get("status","error"), 0) + 1
                time.sleep(1.5)  # respetar CNMV — no saturar
            except Exception:
                resultados["error"] += 1
        return jsonify({"ok": True, "total": len(tickers), **resultados})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
