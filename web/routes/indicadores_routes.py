...
def _calcular_pivot_points(df):
    """Pivot Points semanales — basados en la semana anterior completa.
    El df llega en diario; resampleamos a semanal y cogemos la penúltima semana
    (la última puede estar incompleta si estamos a mitad de semana).
    """
    try:
        import pandas as pd
        df_w = pd.DataFrame({
        ...
        # Semana anterior completa = penúltima fila
        if len(df_w) < 2:
            raise ValueError("Histórico insuficiente para pivots semanales")
        last = df_w.iloc[-2]
    except Exception:
        # Fallback: última vela diaria completa
...
def _detectar_patrones_chartistas(df, n=100):
    ...
    # ── Doble Techo ──────────────────────────────────────────
    # Criterios estrictos:
    # - Dos máximos al mismo nivel ±3%
    # - El 2º máximo NO puede ser más alto que el 1º (sería continuación alcista)
    # - Valle intermedio baja al menos 5% desde los techos
    # - Separación mínima 10 velas, máxima 60 velas
    # Doble Techo: tolerancia ≤2% óptimo, hasta 2.5% permitido
    # Separación: 15-40 velas. Valle intermedio: 7-10% mínimo
    if len(maximos) >= 2:
        m1, m2 = maximos[-2], maximos[-1]
        p1, p2 = highs[m1], highs[m2]
        nivel_medio = (p1 + p2) / 2
        separacion = m2 - m1
        diff_pct = abs(p1 - p2) / nivel_medio
        if (diff_pct <= 0.025 and m2 > m1
                and p2 <= p1 * 1.025         # 2º máximo al mismo nivel ±2.5%
                and p2 >= p1 * 0.975
                and 15 <= separacion <= 40): # separación 15-40 velas
            valle = min(lows[m1:m2+1])
            profundidad = (nivel_medio - valle) / nivel_medio
            # Valle intermedio mínimo 7%
            if profundidad >= 0.07:
                neckline = round(float(valle), 2)
                objetivo = round(float(neckline - (nivel_medio - neckline)), 2)
    ...
    # ── Doble Suelo ──────────────────────────────────────────
    # Criterios estrictos:
    # - Dos mínimos al mismo nivel ±3% (lows tienen más ruido que closes)
    # - El 2º mínimo NO puede ser más bajo que el 1º (sería continuación bajista)
    # - Pico intermedio sube al menos 5% desde los suelos
    # - Separación mínima 10 velas, máxima 60 velas
    # Doble Suelo: tolerancia ≤2% óptimo, hasta 2.5% permitido
    # Separación: 15-40 velas. Rebote intermedio: 7-10% mínimo
    if len(minimos) >= 2:
        ...
        diff_pct = abs(p1 - p2) / nivel_medio
        if (diff_pct <= 0.025 and m2 > m1
                and p2 >= p1 * 0.975         # 2º mínimo al mismo nivel ±2.5%
                and p2 <= p1 * 1.025
                and 15 <= separacion <= 40): # separación 15-40 velas
            pico = max(highs[m1:m2+1])
            rebote = (pico - nivel_medio) / nivel_medio
            # Rebote intermedio mínimo 7%
            if rebote >= 0.07:
    ...
    # Criterios estrictos:
    # - Cabeza al menos 3% más alta que cada hombro
    # - Hombros similares ±5% (tol*2.5)
    # - Hombros AMBOS por debajo de la cabeza
    # - Separación mínima 10 velas entre cada punto
    # - Valle entre H1-Cabeza y Cabeza-H2 usando lows reales
    mejor_hch = None
    if len(maximos) >= 3:
        for i in range(len(maximos) - 2):
            h1, c, h2 = maximos[i], maximos[i+1], maximos[i+2]
            ph1, pc, ph2 = highs[h1], highs[c], highs[h2]
            # Cabeza al menos 3% superior a cada hombro
            if pc <= ph1 * 1.03 or pc <= ph2 * 1.03:
                continue
            # Hombros similares ±5%
            if abs(ph1 - ph2) / max(ph1, ph2) > tol * 2.5:
                continue
            # Separación mínima 10 velas
            if (c - h1) < 10 or (h2 - c) < 10:
                continue
            # Valles reales usando lows
            idx_v1 = h1 + int(np.argmin(lows[h1:c+1]))
            idx_v2 = c + int(np.argmin(lows[c:h2+1]))
            v1 = lows[idx_v1]
            v2 = lows[idx_v2]
            neckline = round(float((v1 + v2) / 2), 2)
            # La neckline debe estar claramente por debajo de los hombros (al menos 2%)
            if neckline >= min(ph1, ph2) * 0.98:
                continue
            objetivo = round(float(neckline) - (pc - neckline), 2)
            confirmado = precio_actual < neckline
            amplitud = pc - neckline
            sesiones_formacion = h2 - h1
            if mejor_hch is None or h2 > mejor_hch['_h2'] or amplitud > mejor_hch['_amplitud']:
                mejor_hch = {
                    ...
                    'sesiones_formacion': sesiones_formacion,
    ...
    # ── HCH Invertido ─────────────────────────────────────────
    # Criterios estrictos (espejo del HCH):
    # - Cabeza al menos 3% más baja que cada hombro
    # - Hombros similares ±5%
    # - Picos reales usando highs
    mejor_hchi = None
    if len(minimos) >= 3:
        for i in range(len(minimos) - 2):
            h1, c, h2 = minimos[i], minimos[i+1], minimos[i+2]
            ph1, pc, ph2 = lows[h1], lows[c], lows[h2]
            # Cabeza al menos 3% inferior a cada hombro
            if pc >= ph1 * 0.97 or pc >= ph2 * 0.97:
                continue
            # Hombros similares ±5%
            if abs(ph1 - ph2) / max(abs(ph1), abs(ph2)) > tol * 2.5:
                continue
            # Separación mínima 10 velas
            if (c - h1) < 10 or (h2 - c) < 10:
                continue
            # Picos reales usando highs
            idx_v1 = h1 + int(np.argmax(highs[h1:c+1]))
            idx_v2 = c + int(np.argmax(highs[c:h2+1]))
            v1 = highs[idx_v1]
            v2 = highs[idx_v2]
            neckline = round(float((v1 + v2) / 2), 2)
            # La neckline debe estar claramente por encima de los hombros (al menos 2%)
            if neckline <= max(ph1, ph2) * 1.02:
                continue
...
def api_datos():
    ...
    # Validar tf con lista blanca
    TF_VALIDOS = {"1d", "1wk", "1mo"}
    if tf not in TF_VALIDOS:
        tf = "1d"
    ...
    try:
        canales_data = detectar_canales_tendencia(df, n_velas=120)
        canales_shapes      = canales_data.get("shapes", [])
        canales_annotations = canales_data.get("annotations", [])
        canales_info        = canales_data.get("canales", [])
    except Exception:
        canales_shapes = canales_annotations = canales_info = []
    ...
    return jsonify({
        ...
        "canales_shapes":      canales_shapes,
        "canales_annotations": canales_annotations,
        "canales_info":        canales_info,
    })
...
# ══════════════════════════════════════════════════════════════
# CALENDARIO DE EVENTOS (v86)
# ══════════════════════════════════════════════════════════════

@indicadores_bp.route("/eventos/<ticker>", methods=["GET"])
def obtener_eventos_ticker_api(ticker):
    """
    Endpoint para obtener eventos próximos de un ticker.
    
    Query params:
        dias: int (default 30) - ventana de días hacia adelante
    
    Returns:
        JSON con eventos próximos (dividendos, earnings, macro)
    """
    from core.contexto_mercado import obtener_eventos_ticker
    
    try:
        ticker = normalizar_ticker(ticker)
        dias = int(request.args.get('dias', 30))
        
        eventos = obtener_eventos_ticker(ticker, dias_adelante=dias)
        
        # Serializar fechas para JSON
        if eventos.get('dividendo') and eventos['dividendo'].get('fecha_ex'):
            fecha_ex = eventos['dividendo']['fecha_ex']
            if hasattr(fecha_ex, 'strftime'):
                eventos['dividendo']['fecha_ex'] = fecha_ex.strftime('%Y-%m-%d')
