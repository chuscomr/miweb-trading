...
def calcular_soportes_resistencias(df, ventana=10, umbral_agrupacion=0.02, max_niveles=8):
    ...
    - Prioriza niveles más recientes
    - Limita cantidad de niveles mostrados
    
    Args:
        df: DataFrame con OHLCV
        ventana: Ventana para detectar máximos/mínimos locales
        umbral_agrupacion: % para agrupar niveles cercanos (ej: 0.03 = 3%)
        max_niveles: Máximo de niveles a devolver por tipo
    ...
    for i in range(ventana, len(df) - ventana):
        # Máximo local
        if df['High'].iloc[i] == max(df['High'].iloc[i-ventana:i+ventana]):
            maximos.append({
        ...
        # Mínimo local
        if df['Low'].iloc[i] == min(df['Low'].iloc[i-ventana:i+ventana]):
            minimos.append({
    ...
    def agrupar_niveles(niveles, umbral):
        ...
        if grupo_actual:
            ...
            bonus_volumen = sum(1 for n in grupo_actual if n.get('volumen', 0) > 0) * 0.3
            dias_promedio = sum(n['dias_desde_hoy'] for n in grupo_actual) / len(grupo_actual)
            factor_recencia = max(0.5, 1 - (dias_promedio / 365))
            fuerza = (fuerza_base + bonus_volumen) * factor_recencia
    ...
    soportes = [s for s in soportes_agrupados 
                if s['precio'] < precio_actual #* 0.99  # 1% debajo como mínimo
                and s['fuerza'] >= 0.8]
    
    # RESISTENCIAS: Solo las que están ENCIMA del precio actual
    resistencias = [r for r in resistencias_agrupadas 
                    if r['precio'] > precio_actual #* 1.01  # 1% encima como mínimo
                    and r['fuerza'] >= 0.8]
    
    # ===========================
    # ORDENAR Y LIMITAR
    # ===========================
    # Soportes: más cercanos primero (descendente)
    soportes.sort(key=lambda x: x['precio'], reverse=True)
    soportes = soportes[:max_niveles]
    
    # Resistencias: más cercanos primero (ascendente)
