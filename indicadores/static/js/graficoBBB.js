...
class GraficoIndicadores {
    ...
    initEventos() {
        ...
        document.getElementById('btn-limpiar-lineas').addEventListener('click', () => {
            ...
            if (grafico && grafico.layout) {
                // Mantener solo las shapes originales del análisis técnico
                // (soportes, resistencias, fibonacci, divergencias)
                const shapesOriginales = (grafico.layout.shapes || []).filter(shape => {
                    // Mantener líneas punteadas (soportes/resistencias)
                    if (shape.line && shape.line.dash === 'dot') {
                        return true;
                    }
                    // Mantener líneas de fibonacci y divergencias (tienen colores específicos)
    ...
    dibujar(data, indicadores) {
        ...
        Plotly.newPlot('grafico', traces, layout, {
            ...
            modeBarButtonsToAdd: [
                {
                    name: 'Línea tendencia',
                    icon: {
                        width: 500,
                        height: 600,
                        path: 'M0,500 L500,100',
                        transform: 'matrix(1,0,0,1,0,0)'
                    },
                    click: function(gd) {
                        Plotly.relayout(gd, {
                            dragmode: 'drawline',
                            'newshape.line.color': '#3b82f6',
                            'newshape.line.width': 2,
                            'newshape.line.dash': 'solid'
                        });
                    }
                },
                {
                    ...
                    icon: {
                        width: 500,
                        height: 600,
                        path: 'M0,300 L500,300',
                        transform: 'matrix(1,0,0,1,0,0)'
                    },
                    click: function(gd) {
                        // Dibujar línea horizontal
                        const currentLayout = gd.layout;
                        const yValue = prompt('Introduce el precio para la línea horizontal:');
                        if (yValue && !isNaN(yValue)) {
                            const newShape = {
                                ...
                                yref: 'y',
                                x0: 0,
                                y0: parseFloat(yValue),
                                x1: 1,
                                y1: parseFloat(yValue),
                                line: {
                                    color: '#6366f1',
                                    width: 2,
                                    dash: 'solid'
                            ...
                            const shapes = (currentLayout.shapes || []).concat([newShape]);
                ...
                },
                {
                    ...
                    icon: {
                        width: 500,
                        height: 600,
                        path: 'M0,150 L500,250 M0,350 L500,450',
                        transform: 'matrix(1,0,0,1,0,0)'
                    },
                    click: function(gd) {
                        alert('Dibuja la primera línea de tendencia. Luego haz clic en "Copiar paralela" para crear el canal.');
                        Plotly.relayout(gd, {
                            dragmode: 'drawline',
                            'newshape.line.color': '#8b5cf6',
                            'newshape.line.width': 2,
                            'newshape.line.dash': 'solid'
                        });
                    }
                ...
                {
                    name: 'Copiar paralela',
                    icon: {
                        width: 500,
                        height: 600,
                        path: 'M0,200 L500,300 M50,50 L550,150',
                        transform: 'matrix(1,0,0,1,0,0)'
                    },
                    click: function(gd) {
                        const shapes = gd.layout.shapes || [];
                        if (shapes.length === 0) {
                            alert('Primero dibuja una línea de tendencia');
                            return;
                        }
                        
                        // Tomar la última línea dibujada
                        const lastShape = shapes[shapes.length - 1];
                        
                        if (lastShape.type === 'line') {
                            const distance = prompt('Distancia de la paralela (en €, deja vacío para distancia automática):');
                            let offset = 0;
                            
                            if (distance && !isNaN(distance)) {
                                offset = parseFloat(distance);
                            } else {
                                // Distancia automática: 5% del rango vertical
                                offset = Math.abs(lastShape.y1 - lastShape.y0) * 0.5;
                            }
                            
                            const parallelShape = {
                                type: 'line',
                                x0: lastShape.x0,
                                y0: lastShape.y0 + offset,
                                x1: lastShape.x1,
                                y1: lastShape.y1 + offset,
                                xref: lastShape.xref || 'x',
                                yref: lastShape.yref || 'y',
                                line: {
                                    color: lastShape.line.color,
                                    width: lastShape.line.width,
                                    dash: 'dash'
                                },
                                editable: true
                            };
                            
                            Plotly.relayout(gd, { shapes: shapes.concat([parallelShape]) });
                        }
                    }
                },
                {
                    ...
                    click: function(gd) {
                        Plotly.relayout(gd, {
                            dragmode: 'drawrect',
                            'newshape.line.color': '#f59e0b',
                            'newshape.line.width': 2,
                            'newshape.fillcolor': 'rgba(245, 158, 11, 0.1)',
                ...
                },
                {
                    ...
                    icon: {
                        width: 500,
                        height: 600,
                        path: 'M150,50 L350,50 L500,300 L250,550 L0,300 Z',
                        transform: 'matrix(1,0,0,1,0,0)'
                    },
                    click: function(gd) {
                        const shapes = gd.layout.shapes || [];
                        if (shapes.length > 0) {
                            // Filtrar las shapes originales (soportes, resistencias)
                            const shapesOriginales = shapes.filter(s => 
                                s.line && (s.line.dash === 'dot' || 
                                          (s.line.color && (s.line.color.includes('22c55e') || s.line.color.includes('ef4444'))))
                            );
                            
                            // Shapes dibujadas por el usuario
                            const shapesDibujadas = shapes.filter(s => 
                                !(s.line && (s.line.dash === 'dot' || 
                                            (s.line.color && (s.line.color.includes('22c55e') || s.line.color.includes('ef4444')))))
                            );
                            
                            if (shapesDibujadas.length > 0) {
                                shapesDibujadas.pop();
                                Plotly.relayout(gd, { shapes: [...shapesOriginales, ...shapesDibujadas] });
                            }
                        }
                    }
                },
    ...
    anadirNivelesSR(soportes, resistencias) {
        ...
        if (shapes.length > 0) {
            Plotly.relayout('grafico', { shapes: shapes });
        }
    }
