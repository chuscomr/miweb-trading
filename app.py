...
def swing_breakout():
    ...
    try:
        ...
        resultado = detectar_breakout_swing(ticker)
        ...
        if resultado and resultado.get("valido"):
            ...
            capital_invertido = acciones * resultado['entrada']
            beneficio_potencial = resultado['objetivo'] - resultado['entrada']
            ...
            contexto.update({
                'señal': 'NO OPERAR',
                'motivos': resultado.get("motivos") if isinstance(resultado, dict) and resultado.get("motivos") else [
    {'ok': False, 'texto': '❌ No se pudo evaluar condiciones técnicas'}
]
...
def swing_pullback():
    ...
    try:
        ...
        resultado = detectar_pullback_swing(ticker)
        precios, volumenes, fechas, precio_actual = obtener_precios(ticker, cache, periodo="6mo")
        contexto_mercado = contexto_ibex(cache)
